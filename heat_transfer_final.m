...
labels = {};
for i = 1:6
    indices(i) = floor(i*length(T_thickness)/12);
    time = indices(i)/length(T_thickness) * total_time;
    labels{i} = ['t_{' num2str(time) ' s}'];
end
labels{7} = ['t_{' num2str(dt) ' s}'];
%% Plotting along thickness
figure(2)
plot(x, T_thickness{1})
hold on
plot(x, T_thickness{indices(1)})
hold on
plot(x, T_thickness{indices(2)})
hold on
plot(x, T_thickness{indices(3)})
hold on
plot(x, T_thickness{indices(4)})
hold on
plot(x, T_thickness{indices(5)})
hold on
...
legend(labels{7}, labels{1},labels{2},labels{3},labels{4},labels{5},labels{6},'T_{payload}')
set(gcf,'color','w');
